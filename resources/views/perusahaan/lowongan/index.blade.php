  ...
  <div class="job_filter_listing_wrapper jb_cover job-section">
    <div class="container">
      ...
      <form class="toolbar" method="get">
        <div class="inputs">
          <input name="q" class="form-control" style="min-width:260px"
        ...
        <div class="table-wrap">
          <table class="table align-middle pretty-table mb-0">
            ...
            <tbody>
              @forelse($items as $l)
                @php
                  $statusClass = $l->status==='published' ? 'pill--pub' : ($l->status==='pending' ? 'pill--pen' : 'pill--cls');
                ...
                <tr>
                  ...
                  <td>
                    <div class="chips">
                      <span class="chip chip--total">Total <span class="cnt">{{ $tot }}</span></span>
                      <span class="chip chip--sent">Dikirim <span class="cnt">{{ $dik }}</span></span>
                      <span class="chip chip--proc">Diproses <span class="cnt">{{ $dip }}</span></span>
                      <span class="chip chip--ok">Diterima <span class="cnt">{{ $dit }}</span></span>
                      <span class="chip chip--rej">Ditolak <span class="cnt">{{ $tol }}</span></span>
                    </div>
                  </td>
                  ...
                  <td class="text-end">
                    <div class="action-dropdown">
                      ...
                      <div class="action-menu dropdown-menu">
                        <a class="dropdown-item" href="{{ route('perusahaan.lamaran.index', $l) }}">
                          <i class="fas fa-users"></i> Lihat Pelamar
                        </a>

                        <a class="dropdown-item" href="{{ route('perusahaan.lowongan.edit', $l) }}">
                          <i class="fas fa-pen"></i> Edit
                        </a>

                        <form action="{{ route('perusahaan.lowongan.toggle', $l) }}"
                              method="POST" class="m-0 p-0">
                          @csrf @method('PATCH')
                          @php $canToggle = in_array($l->status,['published','closed']); @endphp
                          <button type="submit" class="dropdown-item" @disabled(!$canToggle)>
                            <i class="fas {{ $l->status==='published' ? 'fa-lock' : 'fa-unlock' }}"></i>
