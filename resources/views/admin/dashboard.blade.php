            ...
            <div class="row g-3 mb-3 mt-2">
                <div class="col-md-2 col-6">
                    <div class="stat-card">
                        ...
                        <div class="num">{{ $stat['pengguna'] }}</div>
                    </div>
                </div>
                <div class="col-md-2 col-6">
                    <div class="stat-card">
                        <div class="label">Pelamar</div>
                        <div class="num">{{ $stat['pelamar'] }}</div>
                    </div>
                </div>
                <div class="col-md-2 col-6">
                    <div class="stat-card">
                        <div class="label">Perusahaan</div>
                        <div class="num">{{ $stat['perusahaan'] }}</div>
                    </div>
                </div>
                <div class="col-md-2 col-6">
                    <div class="stat-card">
                        <div class="label">Lowongan Aktif</div>
                        <div class="num">{{ $stat['low_aktif'] }}</div>
                    </div>
                </div>
                <div class="col-md-2 col-6">
                    <div class="stat-card">
                        <div class="label">Lowongan Pending</div>
                        <div class="num">{{ $stat['low_pending'] }}</div>
                    </div>
                </div>
                <div class="col-md-2 col-6">
                    <div class="stat-card">
            ...
            <div class="row g-3">
                ...
                <div class="col-lg-6">
                    <div class="jb-card">
                            ...
                            <table class="table align-middle pretty-table mb-0">
                                ...
                                <tbody>
                                    @forelse($jobsLimited as $l)
                                        ...
                                        @endphp
                                        <tr>
                                            <td class="fw-semibold">{{ \Illuminate\Support\Str::limit($l->judul, 42) }}
                                            </td>
                                            <td class="text-muted">
                                                {{ \Illuminate\Support\Str::limit($l->perusahaan->nama_perusahaan ?? '-', 34) }}
                                            </td>
                                            <td><span
                                                    class="badge-pill {{ $cls }}">{{ ucfirst($l->status) }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="3" class="text-center text-muted p-4">Belum ada data.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div class="px-3 py-2 text-end">
                            <a href="{{ route('admin.lowongan.index') }}" class="btn btn-light btn-xs border">Lihat
                                semua</a>
                        </div>
                    </div>
                </div>
                                    ...
                                    @forelse($appsLimited as $a)
                                        ...
                                        <tr>
                                            <td class="fw-semibold">
                                                {{ \Illuminate\Support\Str::limit($a->lowongan->judul ?? '-', 36) }}</td>
                                            <td class="text-muted">
                                            ...
                                            <td><span class="badge-pill {{ $cls }}">{{ ucfirst($st) }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center text-muted p-4">Belum ada data.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div class="row g-3 mt-3">
                    <div class="col-12">
                        <div class="jb-card">
                            <div class="jb-hd d-flex justify-content-between align-items-center">
                                <h1 class="mb-0">Aktivitas Terbaru</h1>
                                <div class="small text-white-50">{{ $aktivitasTerbaru->count() }} entri</div>
                            </div>
                                        ...
                                        @forelse($aktivitasTerbaru as $log)
                                            <tr>
                                                <td>
                                                    ...
                                                    <div class="small text-muted">{{ $log->action }}</div>
                                                </td>
                                                <td>
                                                    <div class="fw-semibold">{{ $log->user->name ?? '-' }}</div>
                                                    <div class="small text-muted">{{ $log->role ?? '-' }}</div>
                                                </td>
                                                <td>{{ $log->lowongan->judul ?? '-' }}</td>
                                                <td>{{ $log->perusahaan->nama_perusahaan ?? '-' }}</td>
                                                <td>{{ $log->pelamar->name ?? '-' }}</td>
                            ...
                            <div class="px-3 py-2 text-end">
                                {{-- nanti bisa diarahkan ke halaman log lengkap --}}
                                <a href="{{ route('admin.aktivitas.index') }}" class="btn btn-light btn-xs border">
                                    Lihat log lengkap
                                </a>
                            </div>
                        </div>
